...
def generate_seed(num_instance):
    ...
    #print(seeds)
    return seeds
def func(name,seed, dirname, num_professions, setting, i,variant):
    cmd=f'python num_professions.py -alg {name}  -p {num_professions} -set {setting} -index {i} -dir {dirname} -seed {seed} -variant {variant}'
    print(cmd)
    os.system(cmd)
        ...
        for i in range(args.instances):
            for setting in settings:
                ...
                for name in algorithms:
                    pool.apply_async(func, (name,seeds[i],dirname, num_professions, setting, i,args.variant,))
